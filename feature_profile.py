...
from seg_fm_maps import SegFMMaps
from feat_idx_maps import FeatIdxMaps
import utils

# Manager
class FeatureProfile:
    def __init__(self, table_path, feat_idx_maps=FeatIdxMaps(), seg_fm_maps=SegFMMaps()):
        self.table_path = table_path
        self.feat_idx_maps = feat_idx_maps
        self.seg_fm_maps = seg_fm_maps
        self.loaded = False
    ...
    def load_table(self):
        if not self.loaded:
            self.feat_idx_maps = FeatIdxMaps()
            self.seg_fm_maps = SegFMMaps()
...
feature value currently degraded to `-`')
    ...
    def __str__(self):
        ...
        fmlayout = self.feat_idx_maps.get_feat_mtx_layout()
        seg_fm_list = self.seg_fm_maps.to_list()
    ...
    def seg_effective_features(self, segment,\
        ...
        feat_mtx = self.seg_fm_maps.feat_mtx_of(segment)
        if feat_mtx is None:
            print(f'Segment [{segment}] not found in feature profile')
            return ''
        else:
            # Obtain string form of feature matrix
            return self.feat_idx_maps.feat_mtx_string(feat_mtx, ef_mask, pos_only)
    ...
    def compare_seg_features(self, segments_str, show_shared):
        ...
        feat_mtxs = []
        for seg in segments:
            feat_mtx = self.seg_fm_maps.feat_mtx_of(seg)
            if feat_mtx == None:
